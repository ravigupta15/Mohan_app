...
class ResolvedComplaintWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    (refState.complaintModel?.data?[0].resolved?.length??0)>0
    ?ListView.separated(
      separatorBuilder: (ctx,sb){
        return const SizedBox(height: 15,);
      },
      itemCount:  (refState.complaintModel?.data?[0].resolved?.length??0),
      padding: EdgeInsets.only(top: 10,bottom: 20,left:17,right: 17),
      shrinkWrap: true,
      itemBuilder: (ctx,index){
          var model = refState.complaintModel?.data?[0].resolved?[index];
        return Container(
