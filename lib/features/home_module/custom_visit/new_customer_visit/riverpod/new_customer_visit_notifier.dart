...
class NewCustomerVisitNotifier extends StateNotifier<NewCustomerVisitState> {
  NewCustomerVisitNotifier()
      : super(NewCustomerVisitState(
            isLoading: false,
            tabBarIndex: 0,
            currentTimer: 0,
  ...
  checkOverViewValidation(BuildContext context,
    ...
    if (state.selectedProductList.isEmpty) {
      ...
      MessageHelper.showErrorSnackBar(
          context, "Please select product trial type");
    } else {
      bool value = state.selectedProductList
          .any((val) => val.list.any((items) => items.quantity == 0));
      if (value) {
        MessageHelper.showErrorSnackBar(
            context, "Quantity should not be empty");
  ...
  setResumeData(BuildContext context, VisitItemsModel model) {
    ...
    state = state.copyWith(
      ...
      selectedCustomerType:
          model.customerType.toString().toLowerCase() == "new" ? 0 : 1,
      visitStartDate: model.visitStart,
      channelParterName: model.channelPartner,
      selectedVisitType:
          model.customerLevel.toString().toLowerCase() == "primary" ? 0 : 1,
...
state = state.copyWith(
  uploadedImageList: [
    ...state.uploadedImageList,  // Spread the previous list
    if (model.imageUrl != null) ...list,  // Append the items from model.imageUrl if it's not null
  ],
);
    ...
    trialPlanAppointmentDate = model.appointmentDate ?? '';
    if((model.trialType ?? '').toString().toLowerCase() == 'item'){
    trailItems = (model.itemTrial ?? []).map((e){
      return {
       ...
       trailItems = (model.productTrial ?? []).map((e){
      return {
        'product':e.product,
      };
    }).toList();
  ...
  channelListApiFunction(
    String searchText,
  ) async {
    final response = await ApiService().makeRequest(
        apiUrl: "${ApiUrls.channelPartnerurl}?search_text=$searchText",
  ...
  customerInfoApiFunction(
    ...
    if (response != null) {
      state = state.copyWith(
          customerInfoModel: CustomerInfoModel.fromJson(response.data));
    }
  }
  ...
  unvCustomerApiFunction(String searchText) async {
    ...
    final response = await ApiService().makeRequest(
        apiUrl: "${ApiUrls.unvCustomerUrl}?search_text=$searchText",
        method: ApiMethod.get.name);
    if (response != null) {
      state = state.copyWith(
          unvCustomerModel: UNVCustomerModel.fromJson(response.data));
    }
  }

  Future customerAddressApiFunction(
      BuildContext context, String searchText, String type) async {
    ShowLoader.loader(context);
    final response = await ApiService().makeRequest(
        apiUrl:
            "${ApiUrls.getCustomerAddressUrl}?customer=$searchText&verific_type=$type",
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
  ...
  productApiFunction(String searchText) async {
    ...
    final response = await ApiService().makeRequest(
        apiUrl: '${ApiUrls.productUrl}?search_text=$searchText',
        method: ApiMethod.get.name);
    if (response != null) {
      state =
          state.copyWith(productModel: ProductModel.fromJson(response.data));
      print(state.productModel?.data);
    }
  }

  Future productItemApiFunction(BuildContext context,
      {required String productTitle, String itemCategory = "MP"}) async {
    state = state.copyWith(productItemModel: null);
    ShowLoader.loader(context);
    final response = await ApiService().makeRequest(
        apiUrl:
            '${ApiUrls.productItemUrl}?product=$productTitle&item_category=$itemCategory',
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
      state = state.copyWith(
          productItemModel: ProductItemModel.fromJson(response.data));
  ...
  competitorApiFunction(BuildContext context, String searchText) async {
    ShowLoader.loader(context);
    final response = await ApiService().makeRequest(
        apiUrl: "${ApiUrls.competitorUrl}?search_text=$searchText",
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
      state = state.copyWith(
          competitorModel: CompetitorModel.fromJson(response.data));
    }
  }
  ...
  Future itemsApiFunction(BuildContext context, String searchText) async {
    ...
    final response = await ApiService().makeRequest(
        apiUrl:
            '${ApiUrls.salesItemVariantUrl}?item_template=$searchText&item_category=',
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
  ...
  void detailsBack(List list) {
    ...
    trailItems = list[2];
    trialPlanAppointmentDate = list[3];
  }

  createProductApiFunction(BuildContext context,
      {required String actionType, required String route}) async {
    ShowLoader.loader(context);
    List<Map<String, dynamic>> formattedData = state.selectedProductList
        .map((e) {
    ...
    String unvCustomer = state.selectedCustomerType == 0
        ? customerNameController.text
        : selectedVerificationType.toLowerCase() == 'verified'
            ? ''
            : selectedExistingCustomer;

    String unvCustomerName = state.selectedCustomerType == 0
        ? customerNameController.text
        : selectedVerificationType.toLowerCase() == 'verified'
            ? ''
            : customerNameController.text;

    String customer = state.selectedCustomerType == 0
        ? ''
        : selectedVerificationType.toLowerCase() == 'verified'
            ? selectedExistingCustomer
            : '';

    String customerName = state.selectedCustomerType == 0
        ? ''
        : selectedVerificationType.toLowerCase() == 'verified'
            ? customerNameController.text
            : '';
    ...
    final body = {
      ...
      "verific_type": selectedVerificationType.isEmpty
          ? 'Unverified'
          : selectedVerificationType,
      "channel_partner":
          state.selectedVisitType == 1 ? channelPartnerController.text : '',
      ...
      "has_trial_plan":
          state.productTrial == 1 ? state.hasProductTrial : 0, //1 or 0
      ...
      "product_trial":
          state.productTrial == 1 && trialType.toLowerCase() == 'product'
              ? trailItems
              : [],
      "item_trial": state.productTrial == 1 && trialType.toLowerCase() == 'item'
          ? trailItems
          : [], // if trial plan is item,
      "conduct_by":
          state.productTrial == 1 ? trialConductType : '', // if trial plan,
...
// print(body);
    ...
    final response = await ApiService().makeRequest(
        apiUrl: ApiUrls.createCVMurl, method: ApiMethod.post.name, data: body);
    ShowLoader.hideLoader();
    if (response != null) {
      CreateOrderResponseModel model =
          CreateOrderResponseModel.fromJson(response.data);
  ...
  stateApiFunction(BuildContext context, {String searchText = ''}) async {
    ...
    final response = await ApiService().makeRequest(
        apiUrl:
            '${ApiUrls.stateUrl}/?filters=[["state", "like", "%$searchText%"]]&limit=100',
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
  ...
  districtApiFunction(BuildContext context, {required String stateText}) async {
    ...
    final response = await ApiService().makeRequest(
        apiUrl:
            '${ApiUrls.districtUrl}/?fields=["district", "state"]&filters=[["district", "like", "%%"], ["state", "=", "$stateText"]]&limit=100',
        method: ApiMethod.get.name);
    ShowLoader.hideLoader();
    if (response != null) {
      state =
          state.copyWith(districtModel: DistrictModel.fromJson(response.data));
    }
  }

  convertToOrderApiFunction(BuildContext context, String id) async {
    ShowLoader.loader(context);
    final body = {"cvm_id": id};
    final response = await ApiService().makeRequest(
        apiUrl: ApiUrls.convertToOrderUrl,
